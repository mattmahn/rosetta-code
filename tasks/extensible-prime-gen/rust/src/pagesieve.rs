/*
 * from http://rosettacode.org/wiki/Sieve_of_Eratosthenes#Unbounded_Page-Segmented_bit-packed_odds-only_version_with_Iterator
 */
...
fn count_page(lmti: usize, pg: &[u32]) -> i64 {
    ...
    let (lmt, icnt) = if lmti >= pgbts {
        ...
        (
            lstw,
            (CLUT[v & 0xFF] + CLUT[(v >> 8) & 0xFF] + CLUT[(v >> 16) & 0xFF] + CLUT[v >> 24]) as u32,
        )
    };
    let mut count = 0u32;
    for i in 0..lmt {
        let v = pg[i] as usize;
        count += (CLUT[v & 0xFF] + CLUT[(v >> 8) & 0xFF] + CLUT[(v >> 16) & 0xFF] +
                      CLUT[v >> 24]) as u32;
    }
    (icnt + count) as i64
}
...
fn primes_pages() -> Box<Iterator<Item = (u64, Vec<u32>)>> {
    ...
    fn pages_from(lwi: u64, szbts: u64, bpas: Bpas) -> Box<Iterator<Item = (u64, Vec<u32>)>> {
        struct Gen(u64, u64);
        ...
        Box::new(Gen(lwi, szbts).map(move |(lwi, szbts)| {
            make_page(lwi, szbts, &bpas)
        }))
    }
...
pub fn primes_paged() -> Box<Iterator<Item = u64>> {
    fn list_paged_primes(
        cmpstpgs: Box<Iterator<Item = (u64, Vec<u32>)>>,
    ) -> Box<Iterator<Item = u64>> {
        Box::new(cmpstpgs.flat_map(move |(lwi, cmpsts)| {
            let pgbts = (cmpsts.len() * 32) as usize;
            (0..pgbts).filter_map(move |i| if cmpsts[i >> 5] & (1u32 << (i & 31)) == 0 {
...
pub fn count_primes_paged(top: u64) -> i64 {
    if top < 3 {
        if top < 2 { return 0i64 } else { return 1i64 }
    }
    let topi = (top - 3u64) / 2;
    primes_pages()
